...
import MenuItem from '../../components/Dota2/profile/menuItem';
...
const Profile = () => {
    ...
    setMenuPosition(prevPosition => {
        ...
        return Math.max(newPosition, 0); // Prevent going beyond the left boundary
    });
};
...
const moveLeft = () => {
    setMenuPosition(prevPosition => {
        const newPosition = prevPosition + 100;
        const maxPosition = 700; // Adjust according to your menu width
        return Math.min(newPosition, maxPosition); // Prevent going beyond the right boundary
    });
};
	...
	const gameServers = useMemo(() => {
		...
		return (
			<motion.div className='w-full flex flex-col gap-10 px-10'>
				<div className="rounded-[30px] overflow-hidden bg-[#1D2333] pb-10 relative z-20">
					<img
				...
				<div className="w-full h-[80px] rounded-3xl flex justify-between items-center bg-[#1D2333] relative z-20">
					<div className={`h-full w-[43px] flex items-center justify-center`}>
						<div className='text-slate-400 cursor-pointer' onClick={moveLeft}>{`<`}</div>
					</div>
					<div className='max-w-[1400px] h-full overflow-hidden'>
						<div className={`w-[1600px] h-full whitespace-nowrap transition-all duration-300 ease-in-out`} style={{ transform: `translateX(-${menuPosition}px)` }}>
								<MenuItem />
								<MenuItem />
								<MenuItem />
								<MenuItem />
					...
					<div className={`h-full w-[43px] flex items-center justify-center`}>
						<div className='text-slate-400 cursor-pointer' onClick={moveRight}>{`>`}</div>
					</div>
				</div>
